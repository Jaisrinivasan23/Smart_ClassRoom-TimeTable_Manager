...
export default function TimetablePage() {
  ...
  async function fetchSupportingData() {
    try {
      const [coursesRes, facultyRes, roomsRes, departmentsRes] = await Promise.all([
        api.get("/courses"),
        api.get("/faculty"),
        api.get("/rooms"),
        api.get("/departments"),
                        ...
                        >
                          ...
                          <SelectContent className="bg-slate-800/90 backdrop-blur-xl border-slate-600/50 text-slate-200">
                            {departments.map((dept) => (
                              <SelectItem key={dept._id} value={dept.name} className="hover:bg-slate-700/50 focus:bg-slate-700/50">
                                {dept.name} ({dept.code})
                              </SelectItem>
                            ))}
                          </SelectContent>
